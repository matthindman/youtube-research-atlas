...
papers but are not yet ingested into the atlas on this branch. See the
Open Holes section below. [🤖]
...
[LIT] RPS can serve as the first stage in latent-property designs. Liu
et al. use a 6,365-video prefix sample as the baseline, then use title
search, modified Dynamic Time Warping, and manual validation to estimate
near-duplicate video levels (Liu et al. 2015, pp. 4-8). [🤖]

[LIT] Because IDs are pseudo-random strings rather than human-chosen
channel names or titles, ID-space sampling sidesteps Rieder's critique
that random-keyword methods inherit language-specific letter-pattern
and title-distribution bias. ID-space sampling is immune to this
specific source of language bias in a way Bärtl's random-keyword
approach is not (Rieder et al. 2020, § 2.1; Bärtl 2018, pp. 20-22).
[🤖]
...
Rieder et al. 2020 center the problem of knowing whether a crawl is
complete, and Ribeiro and West warn that YouNiverse is "likely to
over-emphasize features associated with English-language channels"
...
8. **Paolillo et al. (2019)**, hybrid search/browse/crawl of 549,383
   channels. Ingest primarily to anchor later critiques of historically
   rich but non-representative crawl frames.
...
- **Papers that use this method:** [[liu_et_al_2015]], [[mcgrady_2023]], [[mcgrady_2025]], [[bartl_2018]]
